...
interface _FilesAgent { id: string; name: string; session_id?: string; last_seen_at: string; project_id?: string; }
const _filesAgents = new Map<string, _FilesAgent>();
...
server.tool("register_agent", "Register an agent session. Returns agent_id. Auto-triggers a heartbeat.", {
  ...
  const existing = [..._filesAgents.values()].find(a => a.name === params.name);
  ...
  const ag: _FilesAgent = { id, name: params.name, session_id: params.session_id, last_seen_at: new Date().toISOString() };
  _filesAgents.set(id, ag);
...
server.tool("heartbeat", "Update last_seen_at to signal agent is active.", {
  ...
  const ag = _filesAgents.get(params.agent_id);
...
server.tool("list_agents", "List all registered agents.", {}, async () => {
  return { content: [{ type: "text" as const, text: JSON.stringify([..._filesAgents.values()]) }] };
});
